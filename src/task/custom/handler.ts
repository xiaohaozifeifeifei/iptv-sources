...
const after_replaced = (value: string, replacers?: ICustomRuleReplacerItem[]) => {
  if (!replacers || !Array.isArray(replacers) || replacers.length === 0) return value;
  ...
  const replacer = replacers?.find((e) => {
    ...
    if (e.type === 'string' && !!e.pattern && e.target && value.includes(e.pattern)) {
      return true;
    }
  ...
  if (!!replacer && replacer.type === 'regexp') {
    return value.replace(new RegExp(replacer.pattern, replacer.flags), replacer.target);
  }
...
export const runCustomTask = () => {
  ...
  files.forEach((f) => {
    ...
    const arr = fs.readFileSync(path.join(m3u_path, `${f}.m3u`), 'utf-8').split('\n');
    const res = [arr[0]];
      ...
      for (let i = 1; i < arr.length; i += 2) {
        ...
        if (!!rr.include && Array.isArray(rr.include)) {
          if (rr.include.includes(name)) {
            ...
            let _url = arr[i + 1];
            if (rr.replacer) {
              const { extinf: extinf_replacer, url: url_replacer } = rr.replacer;
